(ns refining.operation
  "OperationActor -- one petroleum-refining operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor (RefiningAdvisor)
  is sealed into a single node (:advise); its proposal is ALWAYS routed
  through the Refinery Safety Governor (:govern) and the rollout phase gate
  ...
  One graph run = one refining operation (intake -> advise -> govern ->
  decide -> commit | hold | approval). No unbounded inner loop -- each
  operation is auditable and checkpointed. A refinery batch's life is
  advanced by MANY ops (verify / process / yield), each its own
  independent run.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human operator (refinery operator / shift superintendent).
  The approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:unit/process`/`:product/yield` ALWAYS reach this node
  when the governor is clean -- see `refining.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [refining.refiningadvisor :as refiningadvisor]
            [refining.governor :as governor]
...
(defn build
  ...
  `refining.store/Store`).
  opts:
    :advisor      -- a `refining.refiningadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (refiningadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; RefiningAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (refiningadvisor/-advise advisor store request)]
            {:proposal p :audit [(refiningadvisor/trace request p)]})))

      ;; Refinery Safety Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
